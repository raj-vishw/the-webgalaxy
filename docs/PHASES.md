...
- **Adaptive graphics**: `AdaptiveQuality` steps Auto down (max twice per session) after 6 s below 34 fps once the explorer is in control; Help & settings (`?`) offers Auto/High/Medium/Low, the keyboard map, *Replay the entry*, *Show the welcome again* and the list view. Dev-only `PerfMonitor` (P or `?perf`): fps, frame time, draw calls, primitives, resources, heap.
