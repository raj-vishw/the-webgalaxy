# The WebGalaxy — phase-by-phase build notes
...
## Phase 7 — Final polish, performance & production readiness

- **Entry**: loading screen (Creating space → Loading universes → Mapping websites → Connecting the galaxy → Ready, with a percentage) → landing over the live galaxy (title, tagline, *Enter the WebGalaxy*, Enter key) → cinematic flight (4.6 s first visit, 2.3 s returning, 0.6 s reduced motion; skippable) → three-card onboarding for first-timers (skippable, replayable from Help). Universe labels are held back behind the title and revealed during the flight. Preferences in `src/store/settingsStore.ts` (localStorage).
- **Scalability**: `UniverseWebsites` budgets full celestial objects (160/110/60 per tier) to the entered universe and to websites that matter right now (selected, hovered, highlighted, connection endpoints, discovery candidates); everything else is one sprite in `WebsitePoints` per universe — orbiting, distance-faded, search-emphasised, one draw call — and registers a placeholder in `celestialRegistry` so flights and lines still target it. `CameraTransition` retries destination resolution across a few frames while an object is promoted.
- **Adaptive graphics**: the scene is fill-rate bound (soft additive particles + bloom), so `ResolutionGovernor` renders the canvas at a per-tier pixel budget (2.2/1.5/1.0 MP) and moves the device-pixel ratio with the measured frame rate (down fast below 48 fps, up slowly above 57; floor 0.6) — labels and UI are HTML and stay crisp; universe names live in one `UniverseLabelLayer` overlay moved from the frame loop instead of one drei `Html` portal each (that alone was ~20 % of a frame). `AdaptiveQuality` steps Auto down (max twice per session) after 3 s below 34 fps, but only once the governor sits at its floor; Help & settings (`?`) offers Auto/High/Medium/Low, the keyboard map, *Replay the entry*, *Show the welcome again* and the list view. Dev-only `PerfMonitor` (P or `?perf`): fps, frame time, draw calls, primitives, resources, heap.
- **Mobile**: a bottom bar (Back · Search · Discover · More → Explore / Filter / Add / Map / Help) replaces the header links under `sm`; bottom sheets sit above it.
- **Deep links & sharing**: `/website/:slug`, `/universe/:slug` apply after entry; the address bar and history follow the journey (`src/lib/router.ts`); *Share* uses the Web Share API or copies the link. SEO shell: title, description, Open Graph/Twitter cards (`public/social.png`), manifest, robots, `<noscript>` content.
- **Accessibility & fallback**: `?view=list` and no-WebGL browsers get `FallbackGalaxy` — universes, websites, search, links and connections as text; distinct accessible names for every control; reduced motion honoured throughout.
- **Command palette**: the search box runs actions (`>` lists them): random/trending/emerging discovery, filters, minimap, return to galaxy, add a website, help.
- **Copy**: themed empty and error states ("Nothing found in this part of the galaxy.", "This object appears to be travelling alone.", "We haven't discovered a connection yet.", "The galaxy connection was interrupted.").
- **Analytics**: `src/lib/analytics.ts` — abstract provider, anonymous product events mapped from the interaction bus.
- **Backend**: batched relationship loads for `fields=full` (no N+1), `websites(universe_id, is_active)` index, generated `search_vector` + GIN index used by search candidates, 403 for disallowed origins.
- **Build**: vendor chunks split (`three`, `react`, lazy `Effects`); e2e suites in `e2e/` (`npm run test:e2e[:mobile]`); vitest for router/palette; README rewritten; `docs/OPERATIONS.md` (deployment, backups, monitoring, security checklist).
