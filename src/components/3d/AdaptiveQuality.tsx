...
import { renderScale } from '../../lib/renderScale'
 ...
 * and lowers the tier (at most twice per session) when it stays low even
 * after the resolution governor has reached its floor — a tier change
 * remounts the heavy parts of the scene, so it is the last resort. Never
...
export function AdaptiveQuality() {
  ...
  useFrame((_, delta) => {
    ...
    frames.current = 0
    elapsed.current = 0
    if (fps < LOW_FPS && renderScale.dpr <= renderScale.floor + 0.01) {
