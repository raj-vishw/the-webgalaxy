...
import { createSurfaceTexture, glowFor } from '../../../utils/celestial'
...
export function PlanetWebsite({ website, frame, orbit, profile, muted = false }: PlanetWebsiteProps) {
  ...
  const atmosphereUniforms = useMemo(
    () => ({
      uColor: { value: new Color(website.accent).lerp(new Color('#dfe6ff'), muted ? 0.6 : 0.3) },
      uIntensity: { value: 0 },
      uPower: { value: muted ? 3.2 : 2.4 },
    }),
    [website.accent, muted],
  )
  ...
  useFrame((_, delta) => {
    ...
    const full = f.lod === 'full'

    if (surfaceRef.current) {
      surfaceRef.current.visible = full
      surfaceRef.current.rotation.y += orbit.spin * delta * (1 - f.hover * 0.5)
  ...
  return (
    <group ref={tiltRef} rotation={[0, 0, orbit.tilt]}>
      ...
      <ObjectGlow ref={haloRef} color={website.accent} whiten={0.25} scale={muted ? 2.6 : 3.2} />
    </group>
  )
}
